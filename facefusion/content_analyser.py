...
def pre_check() -> bool:
    # Mantenemos el pre_check para no romper dependencias
...
def analyse_stream(vision_frame : VisionFrame, video_fps : Fps) -> bool:
    # Siempre devuelve False sin importar el contenido
    return False
...
def forward(vision_frame : VisionFrame) -> float:
    # Siempre devuelve 0.0 sin importar el contenido
    return 0.0

def prepare_frame(vision_frame : VisionFrame) -> VisionFrame:
    # Mantenemos la función de preparación para no romper dependencias
...
def analyse_image(image_path : str) -> bool:
    # Siempre devuelve False sin importar la imagen
    return False

@lru_cache(maxsize = None)
def analyse_video(video_path : str, start_frame : int, end_frame : int) -> bool:
    # Mantenemos la barra de progreso pero siempre devolvemos False
    video_frame_total = count_video_frame_total(video_path)
    video_fps = detect_video_fps(video_path)
    frame_range = range(start_frame or 0, end_frame or video_frame_total)
    
    # Mostramos la barra de progreso pero con rate siempre en 0
    with tqdm(total = len(frame_range), desc = wording.get('analysing'), unit = 'frame', ascii = ' =', disable = state_manager.get_item('log_level') in [ 'warn', 'error' ]) as progress:
        for _ in frame_range:
            progress.update()
            progress.set_postfix(rate = 0.0)
